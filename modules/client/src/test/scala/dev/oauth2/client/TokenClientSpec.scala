...
class TokenClientSpec extends CatsEffectSuite {
  ...
  test("a code exchange carries the code, verifier and redirect uri") {
    for {
      ...
      _ <- tokens.authorizationCode(
        unsafe(dev.oauth2.core.AuthorizationCode.from("code-1")),
        unsafe(dev.oauth2.core.CodeVerifier.from("dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk")),
        Some(unsafe(dev.oauth2.core.RedirectUri.from("https://client.example/cb"))),
        clientId,
        Some(secret)
      )
      request <- seen.get
  ...
  test("a refresh request carries the refresh token") {
    for {
      seen <- Ref.of[IO, Option[(Map[String, String], Option[String])]](None)
      tokens = new TokenClient[IO](transport(seen, ok(grantBody)), endpoint)
      _ <- tokens.refresh(unsafe(dev.oauth2.core.RefreshToken.from("rt-0")), clientId, Some(secret))
      request <- seen.get
    } yield {
      val (fields, _) = request.get
