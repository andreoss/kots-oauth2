...
class TokenEndpointSpec extends CatsEffectSuite {
  ...
  private def proof(compact: String = TokenUri): String =
    kots.oauth2.jose.Dpop
      .prove(
        kots.oauth2.jose.Fakes.signingKey.alg,
        kots.oauth2.jose.Fakes.signingPair.getPrivate,
        kots.oauth2.jose.Fakes.signingJwk,
        unsafe(kots.oauth2.core.JwtId.from("proof-1")),
        "POST",
        compact,
        Start
      )
  ...
  test("a proven token request mints a key bound token of the dpop type") {
    for {
      ...
      val claims = kots.oauth2.jose.Jwt
        .claims(
        ...
        .toOption
        .get
      assertEquals(claims.jkt, kots.oauth2.jose.Dpop.thumbprint(kots.oauth2.jose.Fakes.signingJwk).toOption)
    }
  }
  ...
  test("a token exchange request is answered with the issued token type") {
    for {
      ...
      val response = result.toOption.get
      assertEquals(response.issuedTokenType, Some(kots.oauth2.core.ExchangeTokenType.AccessToken))
