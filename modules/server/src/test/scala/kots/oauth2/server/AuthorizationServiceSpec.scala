...
import kots.oauth2.core.ResponseType
import kots.oauth2.core.Scope
...
class AuthorizationServiceSpec extends CatsEffectSuite {
  ...
  private def request(
    ...
    AuthorizationRequest(
      ...
      state = unsafe(kots.oauth2.core.State.from("xyz")),
      pkce = challenge
    )
  ...
  test("issuance records the resource of the request on the code") {
    val registered = client(Set(callback), "read")
    val bound = unsafe(kots.oauth2.core.ResourceIndicator.from("https://api.example"))
    for {
      triple <- serviceOf()
