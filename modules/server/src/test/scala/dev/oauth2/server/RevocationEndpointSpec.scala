...
class RevocationEndpointSpec extends CatsEffectSuite {
  ...
  private def token(access: String, refresh: Option[String], owner: ClientId = clientId): TokenRecord =
    TokenRecord(
      accessTokenHash = dev.oauth2.core.AccessTokenHash.of(unsafe(dev.oauth2.core.AccessToken.from(access))),
      refreshTokenHash =
        refresh.map(raw => dev.oauth2.core.RefreshTokenHash.of(unsafe(RefreshToken.from(raw)))),
      grantId = grant,
      clientId = owner,
      subject = unsafe(Subject.from("user-1")),
      scopes = unsafe(dev.oauth2.core.Scopes.parse("read")),
      details = dev.oauth2.core.AuthorizationDetails.empty,
      issuedAt = Start,
  ...
  test("a refresh token is revoked with its whole grant") {
    for {
      ...
      first <- tokens.findByAccess(unsafe(dev.oauth2.core.AccessToken.from("at-1")))
      second <- tokens.findByAccess(unsafe(dev.oauth2.core.AccessToken.from("at-2")))
  ...
  test("an access token is revoked alone and the grant survives") {
    for {
      ...
      revoked <- tokens.findByAccess(unsafe(dev.oauth2.core.AccessToken.from("at-1")))
      kept <- tokens.findByAccess(unsafe(dev.oauth2.core.AccessToken.from("at-2")))
      stored <- grants.find(grant)
    } yield {
      assertEquals(result, Right(()))
